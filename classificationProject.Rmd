...
```{r}
predictions_test_Ensemble <- predict(model_glm,predictions_test_all)
```
...
(stats_Ensemble <- summary_binary(predictions_test_Ensemble,binary = F))
```
...
We need to define a modified summary function for this. 
...
Comment: Neural Network model achieves the best accuracy and other metrics. 

# Evaluation of Performance

We have analysed the predictions using the summary metrics function defined above. Now we will compare with ROC curve and area under ROC. 
...
predictionss <- data.frame(DT_GINI = prob_2_class(prediction_test_GINI_final[,2]),
                  ...
                  Ensemble = ifelse(predictions_test_Ensemble=="NO",0,1),
